...
def build_checkplayer_embed(data, page=0, per_page=7):
    ...
    if page == 0:
        ...
        try:
            ...
            bid = newest.get("battleId") if isinstance(newest, dict) else None
            if bid:
                skey = normalize_hourly_battle_key(bid)
                if db_get_setting(f"mcwv_war_scan_complete_{skey}") != "1":
                    footer_parts.append("global scan incomplete")
        except Exception:
            pass
...
from war_cache import (
    WAR_CACHE_PRE_SCAN_HOURS,
    WAR_CACHE_SCAN_CONCURRENCY,
    WAR_CACHE_PARTICIPANT_MAX_PLACE,
    WAR_CACHE_ROLLING_INTERVAL_SECONDS,
    WAR_CACHE_SCAN_MAX_MINUTES,
    queue_full_scan,
    run_scan_exclusive,
    freeze_mcwv_war_from_last_snapshot,
    auto_cache_war_end,
    _auto_cache_full_scan,
    _rank_one_battle_sql,
    get_cached_player_history,
    get_cached_battle_stats,
    _fetch_clan_contributions,
    fetch_all_clan_names_from_sitemap,
    _insert_raw_contributions,
    _insert_participants,
    _auto_cache_priority_scan,
    get_priority_clan_names,
    war_cache_window_loop,
    _compute_global_ranks_sql,
    _run_global_backfill,
    _scan_status_line,
    _get_scan_lock,
    _open_scan_connection_sync,
    _close_scan_connection_sync,
)
...
async def backfill_global_cmd(interaction: discord.Interaction):
    ...
    if _get_scan_lock().locked():
        return await interaction.followup.send(
            f"A scan is already running ({_scan_status_line()}). Check Cache logs.",
            ephemeral=True,
        )
    ...
    asyncio.create_task(run_scan_exclusive(
        _run_global_backfill(interaction.channel_id),
        label="global-backfill",
        max_minutes=90,
    ))
...
async def backfill_status_cmd(interaction: discord.Interaction):
    ...
    running = _scan_status_line()
    if not db_enabled():
        return await interaction.followup.send(f"Scan: **{running}**\nDatabase not available for stats.", ephemeral=True)
    ...
    def _stats():
        ...
        try:
            ...
            with c.cursor() as cur:
                cur.execute("""
                    SELECT
                        COUNT(*) FILTER (WHERE rank IS NOT NULL) AS ranked_rows,
                        COUNT(*) AS rows
                    FROM cross_clan_player_history
                """)
                ranked_rows, rows = cur.fetchone()
                cur.execute("""
                    SELECT battle_id, MAX(total_contributors)
                    FROM cross_clan_player_history
                    WHERE total_contributors IS NOT NULL
                    GROUP BY battle_id
                    ORDER BY MAX(total_contributors) DESC NULLS LAST
                    LIMIT 5
                """)
                top = cur.fetchall()
            c.commit()
            return int(ranked_rows or 0), int(rows or 0), top
        except Exception:
            if c is not None:
                try:
                    c.rollback()
    ...
    lines = [f"Scan: **{running}**"]
    try:
        ranked_rows, rows, top = await asyncio.to_thread(_stats)
        lines.append(f"Cached rows: **{rows:,}**")
        lines.append(f"Ranked rows: **{ranked_rows:,}**")
        if top:
        ...
        try:
            ...
            pass
        lines.append(f"Stats skipped while the scan writes (`{type(e).__name__}`). Scan is still running.")
    await interaction.followup.send("\n".join(lines), ephemeral=True)
...
async def cachewar(interaction: discord.Interaction, battle_id: str):
    ...
    if _get_scan_lock().locked():
        return await interaction.followup.send(
            f"A scan is already running ({_scan_status_line()}). Wait for Cache logs, then try again.",
            ephemeral=True,
        )
    ...
    queue_full_scan(battle_id, include_participants=True, label=f"cachewar:{battle_id}")
